...
public abstract class CommonLyricsPipeline implements LyricsPipeline {
    ...
    @Value("C:\\Backup\\spark-ml-samples-master\\training-set\\lyrics\\DataSets\\")
    private String lyricsTrainingSetDirectoryPath;

    @Value("C:\\Backup\\spark-ml-samples-master\\training-set\\lyrics\\model")
    ...
    Dataset<Row> readLyrics() {
        Dataset input = readLyricsForGenre(lyricsTrainingSetDirectoryPath, Genre.CLASSIC)
    ...
    private Dataset<Row> readLyricsForGenre(String inputDirectory, Genre genre) {
        Dataset<Row> lyrics = readLyrics(inputDirectory, genre.name().toLowerCase() + "\\tcc_ceds_music.csv");
